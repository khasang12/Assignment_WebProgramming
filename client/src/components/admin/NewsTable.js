...
export function NewsTable() {
    ...
    const initValues = { // Default Values for a News
        id: 0,
        admin_name: '',
        content: '',
        thumbnail: '',
    ...
    const [sortStatus, setSortStatus] = useState({ // Sorting Status
    ...
    useEffect(() => {
        setRecords(
            newsList.filter(({ id, admin_name }) => {
                if (
                    debouncedQuery !== '' &&
                    !`${id} ${admin_name}`.toLowerCase().includes(debouncedQuery.trim().toLowerCase())
                ) {
                    return false;
                }
                return true;
    ...
    const deleteNews = (id) => {
        axios({
            ...
            .then((response) => setNewsList((prev) => prev.filter((item) => item.id !== id)))
            .catch((res) => alert(res));
    };
